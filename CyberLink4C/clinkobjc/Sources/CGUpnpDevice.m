...
#include "../../clinkc/Headers/cybergarage/upnp/cdevice.h"
//#include <cybergarage/upnp/cservice.h>
#include "../../clinkc/Headers/cybergarage/upnp/cservice.h"
//#include <cybergarage/upnp/cicon.h>
#include "../../clinkc/Headers/cybergarage/upnp/cicon.h"
...
+(char*) getsongdata
{
    ...
    openErr = AudioFileOpenURL((__bridge CFURLRef) expURL,
    ...
    thePropertySize = sizeof(fileDataSize);  //kAudioFilePropertyID3Tag kAudioFilePropertyAudioDataByteCount
    
    openErr = AudioFileGetProperty(audioFile, kAudioFilePropertyAudioDataByteCount, &thePropertySize, &fileDataSize);
    NSLog(@"Open err %d",(int)openErr);
    ...
    if (theData) {
        NSLog(@"Data %d %d",dataSize,(int)openErr);
        //OSStatus openErr11  = AudioFileReadBytes(audioFile, false, 0, &dataSize, theData);
        OSStatus openErr11 = AudioFileReadBytes(audioFile, false, 0, &dataSize, theData);
        NSLog(@"Open err after reading %d",(int)openErr11);
        // create an NSData object to hold the buffer data
        NSData* nsdata = [NSData dataWithBytesNoCopy:theData length:dataSize]; //?
        NSLog(@"Data %d   %@",dataSize,nsdata);
