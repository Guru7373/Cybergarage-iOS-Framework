...
*		- Fixed memory leak (SAFE_FREE location_str in cg_upnp_service_mangleurl())
*
...
#include "../../../Headers/cybergarage/upnp/cservice.h"
...
#include "../../../Headers/cybergarage/upnp/cupnp.h"
...
char *cg_upnp_service_getnotifyservicetypent(CgUpnpService *service, char *buf, int bufSize)
{
	cg_log_debug_l4("Entering...\n");
    int aservicetypelen;
    char * pservicetype =  cg_upnp_service_getservicetype(service);
    if(pservicetype == NULL)
    {
        return NULL;
    }
    aservicetypelen = cg_strlen(pservicetype);
    if(bufSize >= aservicetypelen-1)
    {
        cg_strncpy(buf,pservicetype , aservicetypelen);
        buf[aservicetypelen] = '\0';
    }
    else{
        cg_strncpy(buf,pservicetype , bufSize);
        buf[bufSize-1]='\0';
    }
	cg_log_debug_l4("Leaving...\n");
	return buf;
...
char *cg_upnp_service_getnotifyservicetypeusn(CgUpnpService *service, char *buf, int bufSize)
{
	...
	cg_log_debug_l4("Entering...\n");
     ...
     if(cg_upnp_service_getservicetype(service) == NULL)
     {
...
char *cg_upnp_device_getlocationurlnew(CgUpnpDevice *dev, const char *host, char *buf, int bufSize,char * url)
{
    cg_log_debug_l4("Entering...\n");
...
BOOL cg_upnp_service_announcefrom(CgUpnpService *service, const char *bindAddr)
{
    ...
    cg_upnp_service_getnotifyservicetypent(service, ssdpLineBuf, sizeof(ssdpLineBuf));
...
CgUpnpAction *cg_upnp_service_getactionbyname(CgUpnpService *service, const char *name)
{
	...
	if (cg_upnp_service_isparsed(service) == FALSE)
		parseresult = cg_upnp_controlpoint_parsescservicescpd(service);
	if(parseresult)
#endif
    {
