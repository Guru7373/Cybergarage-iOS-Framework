...
#include "../../../Headers/cybergarage/net/curi.h"
...
{
    printf("\na<--property changed \nGet NAME %s = Get Value\n %s",cg_upnp_property_getname(prop),cg_upnp_property_getvalue(prop));
...
{
        CgUpnpService *upnpAVTService = cg_upnp_device_getservicebytype(dev,"urn:schemas-upnp-org:service:AVTransport:1");
   // CgUpnpService *upnpAVTService = [dev getServiceForType:@"urn:schemas-upnp-org:service:AVTransport:1"];
      if(upnpAVTService == NULL)      printf("\n\nService NOT found");
    
    
    cg_upnp_controlpoint_seteventlistener(ctrlPoint, functionPtr);
    
    //cg_upnp_controlpoint_seteventlistener(ctrlPoint, EventListener);
    return cg_upnp_controlpoint_subscribe(ctrlPoint, upnpAVTService,timeout);
}
...
void cg_upnp_controlpoint_adddevicebyssdppacket(CgUpnpControlPoint *ctrlPoint, CgUpnpSSDPPacket *ssdpPkt)
{
    ...
    //printf("\n\na<--cg_upnp_controlpoint_adddevicebyssdppacket taking lock\n\n");
    //printf("\na<--ssdp Pkt=%s",ssdpPkt->dgmPkt->data->value);
    ...
    //Akshay :to prevent crash due to network change 
    if(status != CgUpnpDeviceStatusInvalid)
    {
        if (listener != NULL)
        {
...
void cg_upnp_controlpoint_removedevicebyssdppacket(CgUpnpControlPoint *ctrlPoint, CgUpnpSSDPPacket *ssdpPkt)
{
	...
	cg_log_debug_l4("Entering...\n");
    printf("Javed-->cg_upnp_controlpoint_removedevicebyssdppacket %s", ssdpPkt->dgmPkt->remoteAddress->value);
...
{
    printf("\n\na<--Javed :cg_upnp_controlpoint_ipchanged\n\n");
    ...
    cg_upnp_controlpoint_search(ctrlPoint, CG_UPNP_CLINT_Render);
    cg_upnp_controlpoint_search(ctrlPoint, CG_UPNP_CLINT_Server);
...
static void cg_upnp_controlpoint_ssdplistner(CgUpnpSSDPPacket *ssdpPkt)
{
   ...
   // printf("a<---JAVED:cg_upnp_controlpoint_ssdplistner");
	...
	if (cg_upnp_ssdp_packet_isrootdevice(ssdpPkt) == TRUE) {
        ...
        {
            printf("JAVED --> cg_upnp_ssdp_packet_isalive remote address: %s",ssdpPkt->dgmPkt->remoteAddress->value);
