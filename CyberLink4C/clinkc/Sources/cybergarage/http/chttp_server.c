...
#include "../../../Headers/cybergarage/util/cthread.h"
...
static void cg_http_server_clientthread(CgThread *thread)
{
	...
	while (cg_http_request_read(httpReq, clientSock) == TRUE && cg_thread_isrunnable(thread) == TRUE) {
            //printf("\n cg_http_server_clientthread while");
		...
		if (cg_strcmp(version, CG_HTTP_VER11) == 0)
		{
            ...
            //printf("\n cg_http_server_clientthread while11");
       ...
       // printf("\n cg_http_server_clientthread while22");
		...
		if (cg_http_request_iskeepaliveconnection(httpReq) == FALSE)
		{
         ...
         //printf("\n cg_http_server_clientthread while44");
	}
printf("\n cg_http_server_clientthread while33");
	...
	cg_thread_setuserdata(thread, NULL); //Vishnu keep under BOOLean condition and remove from here
    ...
    // This code frequently crashes. mutex lock referencing free'd memory.
        printf("\n\n **cg_http_server_clientthread Before Mutex");
	cg_http_server_lock(httpServer);
	cg_thread_remove(thread);             //Comment these line and implement above if condition to avoid thread delete crash
	cg_http_server_unlock(httpServer);
...
static void cg_http_server_thread(CgThread *thread)
{
	...
	while (cg_thread_isrunnable(thread) == TRUE) {
                //printf("\n cg_http_server_thread inside While!");
