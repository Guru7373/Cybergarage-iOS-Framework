...
{
    printf("a<--cg_http_serverlist_open\n");
	...
	for (netIf = cg_net_interfacelist_gets(netIfList); netIf; netIf = cg_net_interface_next(netIf)) {
        ...
        printf("a<--netif name =%s,bindAddr=%s,port=%d\n",cg_net_interface_getname(netIf),bindAddr,port);
		...
		if (cg_http_server_open(httpServer, port, bindAddr) == FALSE) {
			/* Theo Beisch - why break off, 
			   ...
			   (at least WINCE does...) */
            cg_http_server_delete(httpServer); //Vishnu
