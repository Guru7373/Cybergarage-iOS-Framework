...
#include "../../../Headers/cybergarage/http/chttp.h"
...
void cg_http_packet_init1(CgHttpPacket *httpPkt)
{
	cg_log_debug_l4("Entering...\n");
...
void cg_http_packet_post_user(CgHttpPacket *httpPkt, CgSocket *sock)
{
	CgHttpHeader *header;
...
void cg_http_packet_post(CgHttpPacket *httpPkt, CgSocket *sock)
{
    ...
    BOOL hasHttpWriteTimeout = FALSE;
    
    // Akshay: On network change this loop runs forever , so returning on network timeout
    ...
    for (header = cg_http_packet_getheaders(httpPkt); header != NULL; header = cg_http_header_next(header)) {
        ...
        // adding return statements on network change
        if(cg_socket_write(sock, name, cg_strlen(name)) == Libre_HTTP_WRITE_TIMEOUT_ERROR)
            return;
        if(cg_socket_write(sock, CG_HTTP_COLON, sizeof(CG_HTTP_COLON)-1) == Libre_HTTP_WRITE_TIMEOUT_ERROR)
            return;
        if(cg_socket_write(sock, CG_HTTP_SP, sizeof(CG_HTTP_SP)-1)== Libre_HTTP_WRITE_TIMEOUT_ERROR)
            return;
        value = cg_http_header_getvalue(header);
        if (value != NULL)
        {
            if(cg_socket_write(sock, value, cg_strlen(value))== Libre_HTTP_WRITE_TIMEOUT_ERROR)
                return;
        }
        
        if(cg_socket_write(sock, CG_HTTP_CRLF, sizeof(CG_HTTP_CRLF)-1)== Libre_HTTP_WRITE_TIMEOUT_ERROR)
            return;
    }
    if(cg_socket_write(sock, CG_HTTP_CRLF, sizeof(CG_HTTP_CRLF)-1)== Libre_HTTP_WRITE_TIMEOUT_ERROR)
        return;
    ...
    if (content != NULL && 0 < contentLen)
    {
        if(cg_socket_write(sock, content, contentLen)== Libre_HTTP_WRITE_TIMEOUT_ERROR)
            return;
    }
...
size_t cg_http_packet_read_chunk(CgHttpPacket *httpPkt, CgSocket *sock, char *lineBuf, size_t lineBufSize)
{
	...
	{
        bytesRead = 0;
        bytesRead= cg_socket_read(sock, (content+readLen), (conLen-readLen));
...
BOOL cg_http_packet_read_body(CgHttpPacket *httpPkt, CgSocket *sock, char *lineBuf, size_t lineBufSize)
{
    ...
    if (0 < conLen) {
        ...
        long currReadBytes=0;
        
        /* Read content until conLen is reached, or tired of trying */
        while (readLen < conLen && tries < 5)
        {
            //added by akshay, network change hang issue.
            currReadBytes =0;
            currReadBytes =cg_socket_read(sock, (content+readLen), (conLen-readLen));
            if (currReadBytes <= 0)
                tries++;
            
            readLen += currReadBytes;
