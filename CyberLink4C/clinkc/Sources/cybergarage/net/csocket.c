...
#include "../../../Headers/cybergarage/net/csocket.h"
...
BOOL cg_socket_connect(CgSocket *sock, const char *addr, int port)
{
    ...
    if (0 <= flag)
    {
        if(fcntl(sock->id, F_SETFL, flag | O_NONBLOCK)== -1)
            return FALSE;
    }
...
ssize_t cg_socket_read(CgSocket *sock, char *buffer, size_t bufferLen)
{
	...
	recvLen = tcp_rcv_dat(sock->id, buffer, bufferLen, TMO_FEVR);
        ...
        if(pfd[0].revents != 0)
            printf("\n\na<--socket error occurred = %d\n",pfd[0].revents);
        
        if(rv == -1)
        {
            printf("\na<-- Socket recive ERROR\n");
            recvLen =-1;
        }
        else if (rv == 0)
        {
...
#define CG_NET_SOCKET_SEND_RETRY_CNT 3
#define CG_NET_SOCKET_SEND_RETRY_WAIT_MSEC 20


size_t cg_socket_write_user(CgSocket *sock, void *cmd, size_t cmdLen)
{
    printf("\n# cg_socket_write");
	ssize_t nSent;
	size_t nTotalSent = 0;
		...
		if (nSent <= 0)
		{
			...
			if (CG_NET_SOCKET_SEND_RETRY_CNT < retryCnt)
			{
				/* Must reset this because otherwise return
				   ...
				   fault and this function loops forever*/
				nTotalSent = 0;
                return Libre_HTTP_WRITE_TIMEOUT_ERROR;
				//break;
			}
...
size_t cg_socket_sendto(CgSocket *sock, const char *addr, int port, const char *data, size_t dataLen)
{
    ...
    //Akshay: settting interface for Multicast packet for WiFi scenario
     cg_socket_setmulticastinterface(sock, NULL);
...
BOOL cg_socket_setmulticastinterface(CgSocket *sock, char *ifaddr)
{
    ...
    optSuccess = setsockopt(sock->id, IPPROTO_IP, IP_MULTICAST_IF, (const char *)&sockaddr.sin_addr, sizeof(sockaddr.sin_addr));
...
BOOL cg_socket_freewindowbuffer(CgSocket *sock)
{
    ...
    if (sock->sendWinBuf != NULL)
        SAFE_FREE(sock->sendWinBuf);
    if (sock->sendWinBuf != NULL)
        SAFE_FREE(sock->recvWinBuf);
